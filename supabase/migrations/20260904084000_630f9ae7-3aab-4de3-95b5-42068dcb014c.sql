revoke all on function public.handle_new_user() from anon, authenticated, public;
revoke all on function public.has_workspace_access(uuid) from anon, public;
revoke all on function public.has_workspace_role(uuid, public.app_role[]) from anon, public;
revoke all on function public.current_workspace_id() from anon, public;
grant execute on function public.has_workspace_access(uuid) to authenticated;
grant execute on function public.has_workspace_role(uuid, public.app_role[]) to authenticated;
grant execute on function public.current_workspace_id() to authenticated;